...
import { getTestReports } from 'apiSdk/test-reports';
import { TestReportInterface } from 'interfaces/test-report';

import { MedicineListPage as ShowCaseList } from 'pages/medicines';

function DashboardPage() {
  const { session } = useSession();

  const patientStat = useSWR<PaginatedInterface<PatientInterface>>(
    () => `/patients`,
    () => getPatients({ limit: 1, offset: 0 }),
  );

  const medicineStat = useSWR<PaginatedInterface<MedicineInterface>>(
    () => `/medicines`,
    () => getMedicines({ limit: 1, offset: 0 }),
  );

  const testReportStat = useSWR<PaginatedInterface<TestReportInterface>>(
    () => `/test-reports`,
    () => getTestReports({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Patients',
      result: patientStat?.data?.totalCount,
      isLoading: patientStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Medicines',
      result: medicineStat?.data?.totalCount,
      isLoading: medicineStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Test Reports',
      result: testReportStat?.data?.totalCount,
      isLoading: testReportStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`medicines`} passHref legacyBehavior>
                  <Button
